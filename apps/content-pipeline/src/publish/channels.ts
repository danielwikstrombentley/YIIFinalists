...
import { channelsFileSchema, type ChannelsFile, type ReleaseChannelName } from '@yii/content-schema';
...
export async function readChannels(root: string): Promise<ChannelsFile> {
  try {
    return channelsFileSchema.parse(JSON.parse(await readFile(channelsPath(root), 'utf8')) as unknown);
  } catch (error) {
...
export async function writeChannels(root: string, channels: ChannelsFile): Promise<void> {
  ...
  await writeFile(temporary, `${JSON.stringify(channelsFileSchema.parse(channels), null, 2)}\n`, 'utf8');
...
export function setChannelVersion(
  ...
  return {
    ...
    history: [
      ...channels.history,
      { type, channel, version, at: new Date().toISOString(), actor },
    ],
  };
}
